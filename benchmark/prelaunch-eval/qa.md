...
- MCP initialize and status calls succeed without inference. Codex installation from the public GitHub npm command succeeded. The installed cache exposes all nine tools and the skill; initialize and status were verified there. Direct TypeSafe onboarding still requires an API key on this host. The earlier Gateway MCP is unchanged.
